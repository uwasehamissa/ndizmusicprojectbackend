...
const isBlocked = (ip) => {
  const attempts = failedAttempts.get(ip);
  return attempts && attempts.count >= MAX_ATTEMPTS && (Date.now() - attempts.lastAttempt) < LOCK_TIME;
};
...
const recordFailedAttempt = (ip) => {
  ...
  setTimeout(() => {
    const current = failedAttempts.get(ip);
    if (current && (Date.now() - current.lastAttempt) > LOCK_TIME) failedAttempts.delete(ip);
  }, LOCK_TIME + 60000);
};

const resetFailedAttempts = (ip) => failedAttempts.delete(ip);
...
const protect = async (req, res, next) => {
  ...
  try {
    ...
    const user = await User.findById(decoded.userId).select('-password -verificationToken -resetPasswordToken -resetPasswordExpire');
    if (!user) {
      recordFailedAttempt(clientIP);
    ...
    if (!user.isVerified) {
      return res.status(403).json({ success: false, message: 'Please verify your email', requiresVerification: true });
    }
    ...
    user.lastActivity = new Date();
    await user.save().catch(err => console.warn('Failed to update activity:', err.message));
    ...
    return res.status(401).json({ success: false, message: msg, requiresLogin: true });
  }
};
...
const requireRole = (...roles) => (req, res, next) => {
  ...
  if (!roles.includes(req.user.status)) return res.status(403).json({ success: false, message: `Access denied. Required roles: ${roles.join(', ')}` });
  next();
};
...
const rateLimitMiddleware = (windowMs = 900000, max = 100) => {
  ...
  return (req, res, next) => {
    ...
    const windowStart = now - windowMs;
    const entries = requests.get(ip) || [];
    const valid = entries.filter(time => time > windowStart);
...
const csrfProtection = (req, res, next) => {
  if (['POST', 'PUT', 'DELETE', 'PATCH'].includes(req.method)) {
    const csrfToken = req.headers['x-csrf-token'] || req.body._csrf;
    if (!csrfToken) return res.status(403).json({ success: false, message: 'CSRF token missing' });
  }
  next();
};

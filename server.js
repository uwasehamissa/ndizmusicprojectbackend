...
//       console.error("2. Is the MongoDB service started?");
//       console.error("3. Connection string:", process.env.MONGODB_URI);
//     }
...
========================== */
app.use(helmet());
app.use(cors()); // optional: { origin: ["https://yourfrontend.com"] }
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
...
app.get("/health", (req, res) => {
  ...
  res.json({
    success: true,
    message: "Server is running",
    database: { status: states[dbState] || "unknown", readyState: dbState },
...
app.use((err, req, res, next) => {
  ...
  if (err.message.includes("timeout") || err.code === "ETIMEDOUT")
    return res.status(504).json({ success: false, message: "Connection timeout", error: err.message });

  if (err.name === "MongoNetworkError" || err.name === "MongoServerSelectionError")
    return res.status(503).json({ success: false, message: "Database connection error", error: err.message });

...
class BookingController {
  ...
  async createBooking(req, res) {
    try {
      ...
      Utils.logger.info(`New booking created: ${booking._id}`);
...
async getBookings(req, res) {
  try {
    ...
    console.log("🔍 Final Filter Applied:", finalFilter);
    ...
    console.log(`📦 Bookings Returned (${bookings.length} items)`);

    // Count total matching documents
    const total = await Booking.countDocuments(finalFilter);

    console.log(`📊 Total matching bookings: ${total}`);
    ...
    console.error("❌ Error fetching bookings:", error); // Console log for debugging
    Utils.logger.error('Error fetching bookings:', error);
    ...
    try {
      ...
      const stats = await Booking.aggregate([
        { $match: filter },
        {
          $facet: {
            totalBookings: [{ $count: 'count' }],
            bookingsByStatus: [{ $group: { _id: '$status', count: { $count: {} } } }],
            bookingsByInstrument: [{ $group: { _id: '$instrument', count: { $count: {} } } }],
            bookingsByExperience: [{ $group: { _id: '$experience', count: { $count: {} } } }],
            bookingsByDate: [
              {
                $group: {
                  _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } },
  ...
  async getUnreadCount(req, res) {
    try {
      const { email } = req.query;
      const filter = { isRead: false, ...(email && { email }) };
  ...
  async sendCustomReminder(req, res) {
    try {
      ...
      Utils.logger.error('Error sending custom reminder:', error);
      res.status(500).json({ success: false, message: 'Error sending custom reminder', error: error.message });
    }
  }
}
